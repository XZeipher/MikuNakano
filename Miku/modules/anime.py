...
from Miku import BOT_USERNAME, SUDOERS, app
...
async def anime(client, message: Message):
    if len(message.command) < 2:
        return await message.reply_video("https://telegra.ph/file/1d93a143ebfee6cd816d8.mp4", "Format : /anime < anime name >")
    jikan = Jikan()
...
async def manga(client, message: Message):
    if len(message.command) < 2:
        return await message.reply_video("https://telegra.ph/file/2efc02fcb4551583f0dc2.mp4","Format : /manga < manga name >")
    jikan = Jikan()
    message.command.pop(0)
    search_result = requests.get(
